...
func (dps *DposService) ReceivePublish(v interface{}) {
	...
	if err != nil {
		dps.logger.Info("Publish Event,PublishBlockFromProto err")
		return
	}
	bs := BlockSource{
		block:     p.Blk,
...
func (dps *DposService) ReceiveConfirmReq(v interface{}) {
	...
	e := v.(p2p.Message)
	dps.logger.Info("dpos", e.Data())
	r, err := protos.ConfirmReqBlockFromProto(e.Data())
	if err != nil {
		dps.logger.Error("ConfirmReq Event,ConfirmReqBlockFromProto err")
		return
	}
...
func (dps *DposService) ReceiveConfirmAck(v interface{}) {
	...
	if err != nil {
		dps.logger.Info("ConfirmReq Event,ConfirmAckBlockFromProto err")
		return
	}
	bs := BlockSource{
		block:     ack.Blk,
...
func (dps *DposService) onReceiveConfirmAck(e p2p.Message, ack *protos.ConfirmAckBlock) {
	...
	if !dps.cache.Has(e.Hash()) {
		...
		err := dps.cache.Set(e.Hash(), "")
		if err != nil {
			dps.logger.Errorf("Set cache error [%s] for block [%s] with publish message", err, ack.Blk.GetHash())
		}
	}
}
